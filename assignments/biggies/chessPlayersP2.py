...
def create_b_year_dict(name_values_dict):
    b_year_dict = {}
    for key, value in name_values_dict.items():
        b_year = value[3]
        if b_year in b_year_dict:
            b_year_dict[b_year].append(key)
        else:
            b_year_dict[b_year] = [key]
    return b_year_dict
...
def main():       
    ...
    #3 process file data
    b_year_dict = create_b_year_dict(chess_player_dict)
    ...
    print_result(chess_player_dict, b_year_dict)
